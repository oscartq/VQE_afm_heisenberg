...
from expectation import get_expectation_afm_heisenberg_lattice_3p, AFMHeisenbergLatticeArgs
from optimization import optimize_by_gradient_descent_multiprocess, optimize_by_gradient_descent, optimize_by_lbfgsb_3p
...
def main(): #Main function
    ...
    if optmization == "gradient-descent":
        ...
        for p in p_list:
            ...
            for length in length_list:
                ...
                gamma, beta = optimize_by_gradient_descent_multiprocess(
                    function=partial(get_expectation_afm_heisenberg_lattice_3p, function_args=function_args),
        ...
        for p in p_list:
            ...
            initial_beta = np.array([0.6 for _ in range(p)])
            initial_alpha = np.array([0.6 for _ in range(p)])
            ...
            for length in length_list:
                ...
                gamma, beta, alpha = optimize_by_lbfgsb_3p(
                    function=partial(get_expectation_afm_heisenberg_lattice_3p, function_args=function_args),
                    initial_gamma=initial_gamma,
                    initial_beta=initial_beta,
