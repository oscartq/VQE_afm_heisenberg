...
for i, number_l in enumerate(number_l_list):
    ...
    plt.plot(list(energy_per_length_values.keys()), list(energy_per_length_values.values()),
             marker=markers[i % len(markers)], linestyle=linestyles[i % len(linestyles)], label=f'L = {number_l}', clip_on=False)
...
plt.ylim(0.9, 1.0)  # Set the limits of the y-axis
plt.title(f"Relative energy vs. p-number\n{csv_prefix}, limited y-axis", fontsize=16)  # Font size of the title
